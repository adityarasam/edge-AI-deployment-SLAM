...
| Pipeline Architecture | Model Category | Acc@1px (%) | Acc@3px (%) | Acc@5px (%) | AUC@1px (%) | Detect Time (ms) | Match Time (ms) | Total Pipeline (ms) |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **Fast BRISK + BF** | Classical Binary | `20.89%` | `26.76%` | `28.34%` | `13.38%` | `10.85 ms` | `1.03 ms` | `11.87 ms` |
| **Fast ORB + BF** | Classical Oriented | `25.56%` | `45.33%` | `49.38%` | `17.73%` | `17.03 ms` | `1.14 ms` | `18.17 ms` |
| **SuperPoint + BF** | Heavy Deep CNN | `35.47%` | `60.14%` | `65.55%` | `21.42%` | `1957.17 ms` | `7.11 ms` | `1964.28 ms` |
| **XFeat (INT8) + BF** | Quantized CNN | `22.56%` | `41.26%` | `50.58%` | `15.44%` | `131.67 ms` | `3.89 ms` | `135.56 ms` |
| **XFeat (FP32) + LG (FP32)** | FP32 Deep Reference | `34.00%` | `64.82%` | `77.66%` | `21.92%` | `179.44 ms` | `98.02 ms` | `277.46 ms` |
| **XFeat (INT8) + LG (INT8)** | **Edge AI Champion** | **`34.40%`** | **`64.66%`** | **`78.27%`** | **`22.54%`** | **`128.62 ms`** | **`94.08 ms`** | **`222.69 ms`** |
...
| **XFeat (FP32) + LG (FP32)** | `21.92%` | `277.46 ms` | ✅ Qualified ($\ge 20\%$) | `7.90` | `8.97` | High Precision, Larger Model Size |
...
1. **Quantization Denoising Advantage:**
   * **`XFeat INT8 + LighterGlue INT8`** delivers **`22.54% AUC@1px`**, slightly surpassing the FP32 baseline (`21.92%`). The 8-bit discrete grid suppresses low-amplitude sensor noise before attention computation.
2. **8.8x Speedup over Heavy Deep Baselines:**
   * Runs in **`222.69 ms`** compared to **`1964.28 ms`** for SuperPoint on single-core CPU, delivering superior accuracy with an **8.8x latency reduction**.
